...
class LessonsController extends Controller
{
     ...
     public function index()
    {
     ...
     $category = Lesson::orderby('id','DESC')->get();
      //return $category;
        return view('admin.lessons.index', compact('users','category'));
    }
     public function create()
    {
             $teachers = User::where('role_id',2)->orderby('id','DESc')->get();
             $category = Course::orderby('id','DESC')->where('type','self peaced')->get();

        return view('admin.lessons.create',compact('teachers','category'));
    }
...
$Lesson->save(); 
      ...
      public function edit(Request $request,$id)
    {
        ...
        return view('admin.lessons.edit',compact('data','lessons_media','course','lessons_video'));
    }
...
if ($Lesson->published == 1) {
  ...
  if($request->hasfile('downloadable_files'))
     ...
     //   $fileModel = new Lessons_media;
        ...
        $fileModel= Lessons_media::find($request->pdf_media);
   ...
   $path3= $music_file->move($location,$filename3);
        ...
        //$fileModel2 = new Lessons_media;
        $fileModel2= Lessons_media::find($request->audio_media);
          $fileModel2->filename=$filename3;
        $fileModel2->lesson_id=$Lesson->id;
        $fileModel2->url=$path3;
        $fileModel2->name='add_audio';
 ...
 public function deleteMultipleVideo(Request $request)
    {
        $id = $_REQUEST['id'];
        //return $id;
 ...
 public function multiple_downloadable_files(Request $request)
    {
        $id = $_REQUEST['id'];
        //return $id;
    ...
    //    $data = Lessons_media::find($id);
      $delete = Lessons_media::where('id',$id)->delete();
      //  $data =  Lessons_media::find($id);
                return back()->with('flash_success', 'downloadablefile Deleted Successfully!');
