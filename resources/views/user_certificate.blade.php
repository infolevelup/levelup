...
<section class="u-dashboard">
	...
	<?php 
	$course=App\Course::where('id',$user->course_id)->first();
	?>
	@if($i % 2 == 0)
	...
	<div class="col-lg-7 col-md-7 col-sm-8 col-xs-8 course_name_date">
       <h2>{{$course->title}}</h2>
       <span>Date: {{ \Carbon\Carbon::parse($user->updated_at)->isoFormat('MMM Do YYYY')}}</span>
      </div>
	
	<div class="col-lg-5 col-md-5 col-sm-12 col-xs-12 course_id_price text-right">
      <a href="{{url('/certificate/')}}/{{$user->id}}" class="certificate-btn">View Certificate</a>
	 </div>
	 </article>
	</div>
	@else
