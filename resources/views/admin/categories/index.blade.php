           ...
           <thead>
                            ...
                            @foreach($users as $row)   
                                <tr>
                                    ...
                                    <td>@if($row->popular=='1') Popular category
                                        @else
                                        -
                                        @endif
                                    </td>
                                    <td>@if($row->status=='1') Active
                                        @else
                                        InActive
                                        @endif
                                    </td>
                                    <td>
                                       ...
                                       <a href="{{ route('admin.categories.edit', $row->id) }}"class="btn btn-info" data-bgcolor="#3b5998" data-color="#ffffff"><i class="icon-copy fa fa-pencil" aria-hidden="true"></i></a>
                    ...
                    <!--    <button form="resource-delete-{{ $row->id }}" class="btn btn-danger"><i class="icon-copy fa fa-trash" aria-hidden="true" color='red'></i></button>
                        <form id="resource-delete-{{ $row->id }}" action="{{ route('admin.categories.destroy', $row->id) }}" style="display: inline-block;" onSubmit="return confirm('Are you sure you want to delete this item?');" method="post">
                          @csrf
                          @method('DELETE')
                        </form>--->  
                                    </td>
