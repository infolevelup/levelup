...
<?php 
$cour=App\Course::where('id',$carts->course_id)->first();
$cat=App\Category::where('id',$cour->category_id)->first();
								$subtotal[] = $carts->price_id;
...
-->	<a href="{{url('del/')}}/{{$carts->id}}/session_id/{{$carts->session_id}}" class="btn btn-outline-danger"> × Remove</a>
	</td>
</tr>
@endforeach
...
<tfoot>
						<tr>
							...
							<td class="hidden-xs text-center">₹ {{array_sum($subtotal)}}</td>
